...
${paymentSummary}`);
  ...
  return (
    <main>
      ...
      <section className="section container products-section" id="products"><div className="section-heading"><div><p className="kicker">{t.products.kicker}</p><h2>{t.products.heading}</h2></div><div className="product-tabs">{t.products.tabs.map((tab, index) => <button className={activeProductTab === index ? "active" : ""} onClick={() => setActiveProductTab(index)} key={tab}>{tab}</button>)}</div></div>{filteredProducts.length === 0 ? !catalogReady ? null : <div className="empty-state"><Search size={26} /><h3>{t.products.emptyTitle}</h3><p>{t.products.emptyBody}</p></div> : <div className="product-grid">{filteredProducts.map((product) => <ProductCard key={product.id} product={product} liked={liked.includes(product.id)} onToggleWishlist={() => setLiked((current) => current.includes(product.id) ? current.filter((id) => id !== product.id) : [...current, product.id])} onAddToCart={() => addProductToCart(product)} onQuickView={() => { setActiveProduct(product); setQuantity(1); }} />)}</div>}</section>
